...
prtLogl = function(){ # debugging only -- make sure mdpEta & mdpXi are global
...
#######################################################
## PY mix of normal linear regressions

## common set of functions used for both {eta[j]} and {xi[i]}

## use list with
## global vars for data:
## y:  (RxC) R=# patients, C=# proteins
## offset:  (R x C)
## yEta, yXi and yEtaXi: (RxC) copy of y, for (y-offset) for patients (yXi), proteins (..Eta) or both (..EtaXi)
## X:  (Rxp)
##
## parameters:
...
mdpUpdS = function(mdp, niter=mdp$M, ns=10){
